...
pub fn breadcrumb_spawn_system(
    ...
    mut crumb_map: ResMut<BreadCrumbMap>,
    mut query: Query<(&ActorPathState, &ActorPathGoal, With<Actor>)>,
) {
    for (path, goal, _) in query.iter_mut() {
        if let None = crumb_map.0.insert(path.current_location, true) {
...
pub fn breadcrumb_setup_system(
    ...
    let breadcrumb_map = BreadCrumbMap {
        ..Default::default()
    };
    commands.insert_resource(breadcrumb_map);
}
